...
birthdays_messages = config['Messages']['birtdays_message']
...
def generate_message(name):
    greeting = random.choice(greetings).replace('{name}', name)
    image_path = random.choice(glob.glob(images_path))
    return greeting, open(image_path, 'rb')
...
async def send_birthday_messages():
    while True:
        name = check_birthday()
        if name is not None:
            message, image = generate_message(name)
